/* ================================================================
   MODO OSCURO (luna/sol) — App PÚBLICA Jhonny Perdomo
...
(function () {
  ...
  var KEY = (typeof nsKey === 'function') ? nsKey('jp_tema') : 'jp_tema';
  ...
  function metaColor(){ var m=document.querySelector('meta[name="theme-color"]');
    if(!m) return;
    var claro = (getComputedStyle(document.documentElement).getPropertyValue('--brand') || '#007BFF').trim();
    m.setAttribute('content', oscuro() ? '#0F1826' : claro); }
  function aplica(d){
