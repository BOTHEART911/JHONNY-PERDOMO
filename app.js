...
function logout() { LIDER = null; localStorage.removeItem(ACT_KEY); go('login'); }
...
function viewLogin() {
  ...
  app.innerHTML = `
    <div class="login-wrap"><div class="login-card">
      ...
      <button class="link-install" id="goInstall">📲 Instalar la app</button>
      ${footBrand()}
    </div></div>`;
  app.hidden = false; hideSplash(); paintVersion(APP_VERSION_LOADED || (typeof APP_VERSION !== 'undefined' ? APP_VERSION : ''));
  pinBuffer = ''; paintPin();
  ...
  const gi = $('#goInstall'); if (gi) gi.onclick = () => go('instalar');
...
/* ============================================================
   ...
   Si el usuario es líder, el mensaje incluye su N° de Referido
   (código), como en el portal real. Si no, es el refiere genérico.
   ============================================================ */
...
function msgRefiereGeneral() {
  return 'Instala la *App de Jhonny Perdomo* 📲 y regístrate con un clic:\n' + REFIERE_URL;
}
async function viewRefiere(user) {
  app.innerHTML = `${backbar('Refiere por WhatsApp')}<div class="pad stack">
    <div class="card pad stack center">
      ...
      <p class="muted" id="ref-sub">Comparte la app con tus contactos. Entre más seamos, más lejos llegamos.</p>
    ...
    <p class="center small muted">El enlace lleva a instalar y registrarse en la app.</p>
  </div>`;
  app.hidden = false; hideSplash(); $('#backbtn').onclick = () => go('home');

  // ¿es líder? si lo es, usamos el mensaje con su código
  ...
  const mensaje = esLider ? msgRefiereLider(codigo, nombre) : msgRefiereGeneral();
  if (esLider) {
    $('#ref-sub').innerHTML = 'Cuando alguien se registre, pídele que escriba tu <b>N° de Referido</b> para sumarlo a tu grupo.';
    const rc = $('#ref-code'); rc.classList.remove('hidden');
    rc.innerHTML = `<div class="idpill" style="font-size:1.05rem;padding:8px 16px;">Tu N° de Referido: <b>${esc(codigo)}</b></div>`;
  }
  $('#ref-wa').onclick = () => {
...
async function viewLideres(user) {
  ...
  // Gate: confirmar contraseña de líder
  app.innerHTML = `${backbar('Zona de líderes')}<div class="pad stack">
    <div class="card pad stack center">
      <div class="ico" style="width:64px;height:64px;margin:0 auto;border-radius:18px;background:var(--brand-050);display:grid;place-content:center;">${I.star}</div>
      <h2 class="h2">Acceso de líder</h2>
      <p class="muted">Ingresa tu contraseña de líder para administrar tus referidos.</p>
      <label class="field" style="text-align:left;"><span>Contraseña</span>
        <input class="input" id="lg-clave" type="password" placeholder="Tu contraseña" autocomplete="off" /></label>
