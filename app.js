...
const RUTAS_ANCHAS = new Set(['', 'home', 'ideas', 'casa', 'emergencia', 'comercio', 'noticias', 'lideres']);
...
/* Panel principal del líder: menú de sub-secciones.
   ...
   06/08/2026 · ANTES esta función esperaba a 'pub.esProfesional' ANTES de
   escribir una sola línea de HTML. Resultado en PC: el router ya había quitado
   el ancho (la ruta 'lideres' no estaba en RUTAS_ANCHAS), así que se veía el
   Inicio ENCOGERSE y quedarse ahí con el loader de arriba mientras iba y venía
   la llamada, y solo después aparecía la Zona. Dos pasos para una sola cosa.

   Ahora: 'lideres' va en RUTAS_ANCHAS (no se encoge nada), el panel se pinta
   de inmediato con lo que ya se sabe y la pregunta del profesional viaja en
   segundo plano SIN loader (apiSilencio). Si la respuesta cambia lo que hay en
   pantalla, se repinta; si el líder ya se fue a una sub-vista, no se le toca
   nada. Mismo patrón que ES_LIDER/esLiderCargar en Inicio. */
let ES_PROF = { doc: '', val: null };
async function esProfCargar(user) {
  try {
    const pr = await apiSilencio('pub.esProfesional', { documento: user.documento });
    ES_PROF = { doc: user.documento, val: !!(pr && pr.esProfesional) };
  } catch (e) {
    if (ES_PROF.doc !== user.documento) ES_PROF = { doc: user.documento, val: false };
  }
  return ES_PROF.val;
}
const esProfCache = (user) => (ES_PROF.doc === user.documento && ES_PROF.val !== null) ? ES_PROF.val : null;

function liderPanel(user) {
  const cache = esProfCache(user);
  liderPanelPintar(user, cache === true);
  esProfCargar(user).then(v => {
    // solo si cambió Y el panel sigue en pantalla (id propio: #lp-grid)
    if (v !== (cache === true) && $('#lp-grid')) liderPanelPintar(user, v);
  }).catch(() => {});
}

function liderPanelPintar(user, esProf) {
  const items = [
  ...
  app.innerHTML = `${backbar('Zona de líderes')}<div class="pad stack">
    ...
    <div class="menu-grid" id="lp-grid">${items.map(m => `<button class="tile" data-id="${m.id}"><span class="ico">${I[m.icon]}</span><span class="txt"><b>${esc(m.t)}</b><br><span>${esc(m.d)}</span></span></button>`).join('')}</div>
  </div>`;
  app.hidden = false; hideSplash(); appWide(true); $('#backbtn').onclick = () => go('home');
