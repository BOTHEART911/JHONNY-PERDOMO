...
async function viewComercio(user) {
  app.innerHTML = `${backbar('Comerciantes amigos')}
    <div class="pad stack">
      <div class="com-top">
        ...
        <div class="small muted">Elige una categoría y toca los íconos para explorar cada comercio.</div>
        <div class="com-icons">
          <img src="${COM_ICON.ubic}" alt="Ubicación"><img src="${COM_ICON.fb}" alt="Facebook"><img src="${COM_ICON.ig}" alt="Instagram"><img src="${COM_ICON.tt}" alt="TikTok"><img src="${COM_ICON.wa}" alt="WhatsApp"><img src="${COM_ICON.tel}" alt="Llamada">
        </div>
      </div>

      <label class="field"><span>Categoría</span>
        <select class="input" id="com-cat"><option value="" selected disabled>Cargando categorías…</option></select>
      </label>

      <div id="com-spec" class="com-spec hidden">
        <div id="com-cat-chip" class="com-cat-chip"></div>
        <div id="com-spec-text" class="small"></div>
      </div>

      <div id="com-list" class="com-list"></div>
    </div>`;
  app.hidden = false; hideSplash();
  appWide(true); // grid a lo ancho en desktop; vuelve a 480 en móvil
  $('#backbtn').onclick = () => { appWide(false); go('home'); };

  const sel = $('#com-cat');
  // Cargar categorías (solo las que tienen comercios activos)
  try {
    const cats = await api('com.categorias');
    if (!cats || !cats.length) {
      sel.innerHTML = '<option value="" selected disabled>Aún no hay comercios</option>';
      $('#com-list').innerHTML = `<p class="center muted" style="padding:24px 0;">Todavía no hay comerciantes publicados. Vuelve pronto 🛍️</p>`;
      return;
    }
    sel.innerHTML = '<option value="" selected disabled>Selecciona la categoría</option>' +
      cats.map(c => `<option value="${esc(c)}">${esc(c)}</option>`).join('');
  } catch (e) {
    sel.innerHTML = '<option value="" selected disabled>Error al cargar</option>';
    toast('No se pudieron cargar las categorías', 'err');
    return;
  }
  ...
  sel.onchange = async () => {
    ...
    if (!categoria) return;
    const spec = $('#com-spec'), specChip = $('#com-cat-chip'), specText = $('#com-spec-text'), list = $('#com-list');
    list.innerHTML = `<div class="com-loading">${I.store} Cargando comercios…</div>`;
    try {
      const data = await api('com.porCategoria', { categoria });
      // Especificaciones de la categoría
      const cat = data.categoria || categoria, txt = data.especificaciones || '';
      specChip.textContent = cat; specText.textContent = txt;
      spec.classList.toggle('hidden', !(cat || txt));
      // Tarjetas
      renderComercioList(Array.isArray(data.items) ? data.items : []);
    } catch (e) {
      list.innerHTML = `<p class="center muted" style="padding:24px 0;">No se pudo cargar esta categoría.</p>`;
    }
  };
}
...
function renderComercioList(items) {
  ...
  if (!wrap) return;
  if (!items.length) {
    wrap.innerHTML = `<p class="center muted" style="padding:24px 0;">No hay comercios para esta categoría.</p>`;
    return;
  }
  const movil = esMovil();
  wrap.innerHTML = items.map(it => {
    ...
    const acts = [
      ...
      icon((tel && movil) ? ('tel:' + tel) : (tel ? ('tel:' + tel) : ''), COM_ICON.tel, 'Llamar')
    ].join('');
    return `<div class="com-card">
      ...
      ${img ? `<img class="com-card-img" src="${esc(img)}" alt="" loading="lazy" onerror="this.style.display='none'">` : ''}
