...
/* ---------- Cliente API ---------- */
let _apiActivas = 0;
function loaderOn() { _apiActivas++; const b = document.getElementById('topbar-loader'); if (b) b.classList.add('active'); }
function loaderOff() { _apiActivas = Math.max(0, _apiActivas - 1); if (_apiActivas === 0) { const b = document.getElementById('topbar-loader'); if (b) b.classList.remove('active'); } }
...
function pintarMisServicios(user) {
  ...
  $('#ls-body').innerHTML = `
    ...
    <div class="lr-list" id="ms-list">${filtradas.map(s => msCard(s)).join('') || `<div class="card pad center muted">Sin solicitudes en este filtro.</div>`}</div>`;
...
function msCard(s) {
  ...
  return `<div class="rcard" data-fila="${s.fila}" data-search="${esc(s.nombre + ' ' + s.documento + ' ' + s.solicitud)}" style="cursor:pointer;">
...
function msDetalle(user, s) {
  ...
  openSheet(`<div class="grip"></div>
    <div class="rcard-top" style="margin-bottom:10px;"><div class="rc-av">${esc(iniciales(s.nombre))}</div><div class="rc-id"><b>${esc(s.nombre)}</b><span>CC ${esc(s.documento)}</span></div></div>
    ...
    <button class="btn btn-primary btn-block" id="ms-guardar">Guardar</button>
  ...
  const wa = $('#ms-wa'); if (wa) wa.onclick = () => window.open('https://wa.me/57' + onlyDig(s.contacto), '_blank');
}
...
async function liderOlvideClave(user) {
  const ok = await confirmar('Recuperar contraseña', 'Te enviaremos tu contraseña a tu <b>correo</b> y por <b>WhatsApp</b> a tu número registrado. ¿Continuar?');
  if (!ok) return;
  try {
    const r = await api('pub.liderRecuperar', {}, 'POST', { documento: user.documento });
    if (!r.ok) return toast(r.msg || 'No se pudo recuperar', 'err');
    // abre WhatsApp con el mensaje al propio líder (si el backend devolvió el link)
    if (r.waLink) window.open(r.waLink, '_blank');
    celebrar('🔐', '¡Listo!', (r.correo ? 'Enviamos tu contraseña a tu correo' : 'Enviamos tu contraseña') + (r.waLink ? ' y abrimos WhatsApp para ti.' : '.'), 3000);
  } catch (e) { toast('Error de conexión', 'err'); }
}
