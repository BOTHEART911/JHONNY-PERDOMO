...
async function viewIdeas(user) {
  app.innerHTML = `${backbar('Suma tus ideas')}<div class="pad stack"><p class="muted">Comparte tus ideas para Flandes. Llena solo los aspectos que quieras.</p><div class="card pad stack"><div class="ideas-grid">${fieldInfo('Social', 'social', areaEl('i-social', 'Ideas sociales…'))}${fieldInfo('Institucional', 'institucional', areaEl('i-institucional', 'Ideas institucionales…'))}${fieldInfo('Económico', 'economico', areaEl('i-economico', 'Ideas económicas…'))}${fieldInfo('Ambiental', 'ambiental', areaEl('i-ambiental', 'Ideas ambientales…'))}${field('Otros', areaEl('i-otros', 'Otras ideas…'))}</div>
    <div class="ter-block">
...
async function liderPanel(user) {
  ...
  const items = [
    ...
    { id: 'l-clave',       icon: 'card',  t: 'Actualizar contraseña',d: 'Cambia tu clave · agrega o edita tu correo' }
  ];
...
/* ---- ACTUALIZAR CONTRASEÑA ----
   ...
   Cada una tiene su propio Guardar y no se estorban. El correo se lee
   de la sesión (viene de la columna CORREO de LIDERES) y se sobrescribe
   ahí mismo con pub.liderCorreo. */
function liderActualizarClave(user) {
  app.innerHTML = `${backLider('Actualizar contraseña')}<div class="pad stack">
    ...
    <div class="card pad stack">
      <div><b>Tu correo</b><p class="muted small" style="margin:4px 0 0;">Es el medio por el que te enviamos tu contraseña si la olvidas. Puedes agregarlo o cambiarlo cuando quieras.</p></div>
  ...
  $('#cm-save').onclick = async () => {
    ...
    try {
      const r = await api('pub.liderCorreo', {}, 'POST', { documento: user.documento, correo });
      saving(btn, false);
      if (!r.ok) return toast(r.msg || 'No se pudo guardar el correo', 'err');
      if (LIDER) LIDER.correo = r.correo || correo;
      const el = $('#cm-mail'); if (el) el.value = r.correo || correo;
      toast('Correo guardado', 'ok');
    } catch (e) { toast('Error de conexión', 'err'); saving(btn, false); }
  };
