...
const IMPLEMENTADAS = new Set(['tarjeta', 'datos', 'solicitud', 'ideas', 'refiere', 'lideres', 'comercio', 'noticias', 'casa', 'emergencia']);
...
function casaPintar() {
      ...
      <div class="casa-dia-b">
        ...
        <ul class="casa-items">
          ${d.items.map(i => `<li class="casa-item">
            ...
            ${i.horario ? `<span class="casa-item-h">${esc(i.horario)}</span>` : ''}</div>
          </li>`).join('')}
        </ul>
      </div>
    </article>`).join('')
  ...
  /* Iniciar recorrido: en el celular, Waze y Maps; en PC, Waze no sirve. */
  const recorrido = hayMapa ? `
    <section class="casa-ir">
      ${c.direccion ? `<p class="casa-dir">${I.map} ${esc(c.direccion)}</p>` : ''}
      <div class="casa-ir-b">
        <a class="btn btn-primary btn-block" id="casa-maps" href="https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(c.lat + ',' + c.lng)}" target="_blank" rel="noopener noreferrer">${I.map} Iniciar recorrido</a>
        ${esMovil() ? `<a class="btn btn-ghost btn-block" id="casa-waze" href="https://waze.com/ul?ll=${encodeURIComponent(c.lat + ',' + c.lng)}&navigate=yes" target="_blank" rel="noopener noreferrer">Abrir en Waze</a>` : ''}
      </div>
    </section>` : '';
