...
/* Entrada manual a la vista Instalar desde el login. El gate del arranque solo
   aparece si la app NO figura como instalada, y una vez que se instala queda
   marcada para siempre (pwaInstalledFlag). Sin este botón no había forma de
   volver a la vista Instalar: por eso ahora vive en el login, permanente. */
...
const IMPLEMENTADAS = new Set(['tarjeta', 'datos', 'solicitud', 'ideas', 'refiere', 'lideres', 'comercio', 'noticias']);
...
async function liderCompromisos(user) {
  app.innerHTML = `${backLider('Mis compromisos')}<div class="pad stack" id="lc-body">${loadingBox('Cargando…')}</div>`;
  app.hidden = false; hideSplash(); bindBackLider(user);
  try {
...
async function ntQuitarAvisos() {
  const token = ntTokenLocal();
  try { localStorage.removeItem(NT_TOKEN_KEY); } catch {}
  if (token) { try { await api('pub.tokenQuitar', {}, 'POST', { token }); } catch {} }
  try {
    const fb = await ntLoadFirebase(true);
    await fb.messaging().deleteToken();
  } catch {}
...
const NT_REDES = {
  facebook: { txt: 'Ver en Facebook', cls: 'nt-fb' },
  instagram: { txt: 'Ver en Instagram', cls: 'nt-ig' },
  x: { txt: 'Ver en X', cls: 'nt-x' },
  youtube: { txt: 'Ver en YouTube', cls: 'nt-yt' },
  tiktok: { txt: 'Ver en TikTok', cls: 'nt-tt' },
  whatsapp: { txt: 'Abrir en WhatsApp', cls: 'nt-wa' },
  web: { txt: 'Abrir enlace', cls: 'nt-web' }
};

function ntCardHtml(n, nueva) {
  const red = NT_REDES[n.red] || NT_REDES.web;
  ...
  return `<article class="nt-card${nueva ? ' nt-new' : ''}" data-id="${esc(n.id)}">
    ...
    ${n.enlace ? `<a class="nt-link ${red.cls}" href="${esc(n.enlace)}" target="_blank" rel="noopener noreferrer">${I[n.red] || I.share} ${esc(red.txt)}</a>` : ''}
  </article>`;
}
...
function ntAvisosHtml() {
  ...
  if (p === 'granted' && ntTokenLocal()) {
    return `<div class="nt-avisos nt-avisos-on"><div class="nt-ico">${I.news}</div>
      <div><b>Avisos activados 🔔</b><span class="small muted">Te avisamos apenas haya una noticia.</span></div>
      <button class="nt-avisos-btn" id="nt-off">Desactivar</button></div>`;
  }
  if (p === 'denied') {
    return `<div class="nt-avisos nt-avisos-off"><div class="nt-ico">${I.news}</div>
  ...
  return `<div class="nt-avisos"><div class="nt-ico">${I.news}</div>
    <div><b>Activa los avisos</b><span class="small muted">Entérate de cada noticia apenas se publique.</span></div>
    <button class="nt-avisos-btn nt-on" id="nt-on">Activar</button></div>`;
}

function ntPintarAvisos(user) {
  const el = $('#nt-avisos'); if (!el) return;
  el.innerHTML = ntAvisosHtml();
  const on = $('#nt-on'), off = $('#nt-off');
  if (on) on.onclick = async () => { saving(on, true); const ok = await ntActivarAvisos(user); saving(on, false); if (ok) ntPintarAvisos(user); };
  if (off) off.onclick = async () => { saving(off, true); await ntQuitarAvisos(); saving(off, false); ntPintarAvisos(user); };
}
